...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  _pageLoad(HomePageLoadEvent event, emit) async {
    ...
    await LoginHelper.addDevice(
      userId: userData.users!.id.toString(),
  ...
  _changeBottomNavigationBarIndex(
    HomeChangeBottomNavigationItemEvent event,
    emit,
  ) async {
  ...
  _notificationSilent(HomePageNotificationSilentEvent event, emit) async {
    String notificationSilent = await SharedPreferencesUtils.getString(
      key: PreferencesName.notificationSilent,
    );
    if (notificationSilent == "1") {
      isNotificationSilent = false;
